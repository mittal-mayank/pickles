(function checkScriptLoaded() {
    if (scriptLoaded) {
                ...
                if (/\S/.test(inpPostTitleVal)) {
                    if (/\S/.test(inpPostCommVal)) {
                        $.ajax({
                            ...
                            success: () =>
                                (location.href = `/c/${inpPostCommVal}`),
                        });
                    } else
                        raiseValidityMsg(inpPostComm, 'Value cannot be empty!');
                ...
                if (/\S/.test(inpCommNameVal)) {
                    if (/\S/.test(inpCommDescriptionVal)) {
                        $.ajax({
                            ...
                            success: () =>
                                (location.href = `/c/${inpCommNameVal}`),
                        });
                    } else
                        raiseValidityMsg(
                            inpCommDescription,
        ...
        location.href = '#';
    } else setTimeout(checkScriptLoaded, 100);
})();
